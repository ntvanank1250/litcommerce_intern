...
class SQL:
    ...
    def insert_one_row(self, row):
        ...
        """
        val = (row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[9], row[10], row[11], convertDatetime(row[12]))
        return self.__dbProvider.add_data(sql, val)

    def getAll(self):
        sql = """
            SELECT customer_id, firstname, lastname, companyname, billingaddress1, billingaddress2, city, state, postalcode, country, phonenumber, emailaddress, createddate
            FROM customer
        """
